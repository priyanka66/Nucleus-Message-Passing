...
Thread::Thread(char* threadName)
{
    ...
    PID = id++;
    space = NULL;
    messageQueue = new List<MessageBuffer*>;
    waitMsg = FALSE;
    waitAns = FALSE;
}
...
bool Thread::isMessageQueueEmpty() {
    return messageQueue->IsEmpty();
}


MessageBuffer* Thread::getMessage() {
...
MessageBuffer* Thread::removeMessage() {
   if (!messageQueue->IsEmpty()) {
       return messageQueue->RemoveFront();
   }  else {
       return NULL;
   }
}

void Thread::addMessage(MessageBuffer* message){
    if (!messageQueue->IsInList(message))
