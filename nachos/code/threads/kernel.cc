...
Kernel::Kernel(int argc, char **argv)
{
    ...
    msgBuffer = new Bitmap(maxBufferSize);
    quantum = 10000; 
    maxMsgs=5;
    debugUserProg = FALSE;
...
//    maxBufferSize = 10;
    messagebuffer =  new MessageBuffer[maxBufferSize];
    for (int k=0;k<maxBufferSize;k++) {
        messagebuffer[k].initialiseMessage();
        messagebuffer[k].setBufferID(k);
        messagebuffer[k].setIsEmpty(TRUE); 
    }
#ifndef FILESYS_STUB
    formatFlag = TRUE;
#endif
